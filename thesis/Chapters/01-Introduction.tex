...
\begin{figure}[htpb]
	...
	\begin{tikzpicture}
		\begin{axis}[height=0.4\textwidth, width=0.6\textwidth,
			...
			\node[anchor=west, align=left] at (axis cs:2.5, 1) {$r_c$};
		...
		\end{axis}
	\end{tikzpicture}
	\caption{An illustration of the potential well of the 12-6 LJ potential, with the minimum of $-\varepsilon$ at $r_\text{min}=\sigma\sqrt[6]{2}$, zero-crossing at $\sigma$ and cutoff radius $r_c$. The figure is based on Lenhard et al. \cite{Lenhard2024}}
	\label{fig:lj_graph}
\end{figure}

% TODO: mark r=1\sigma as zero-crossing point
% TODO: Introduce reduced units?
